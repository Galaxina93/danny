                ...
                @if($showArchive)
                    @if($this->hasPermission('archive'))
                        <span title="Wiederherstellen">
                            <x-heroicon-o-arrow-path wire:click="restoreItem('{{ $item->id }}')" class="hero-icons" />
                        </span>
                    @endif
                @else
                    @if($this->hasPermission('edit'))
                        <span title="Bearbeiten">
                            <x-heroicon-o-pencil-square wire:click="edit('{{ $item->id }}')" class="hero-icons" />
                        </span>
                    @endif
                    @if($this->hasPermission('delete'))
                        <span title="Archivieren">
                            <x-heroicon-o-archive-box wire:click="confirmDelete('{{ $item->id }}')" class="hero-icons" />
                        </span>
                    @endif
                    @if($this->hasPermission('force_delete'))
                        <span title="Löschen">
                            <x-heroicon-o-trash wire:click="confirmForceDelete('{{ $item->id }}')" class="hero-icons" />
                        </span>
                    @endif
                @endif
            </div>
